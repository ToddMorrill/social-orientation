"""
Modified from: https://github.com/openai/openai-cookbook/blob/main/examples/api_request_parallel_processor.py

Crucially, this modified version checks for completed requests to avoid
repeating work (and paying for it again). This is particularly useful when your
program crashes (e.g., due to a lost connection, etc.) and you need to continue
running a job.
...
python api_request_parallel_processor.py \
  ...
  --max_attempts 10 \
  --logging_level 20 \
> logs/gpt/gpt4_train-long.log 2>&1 &
...
async def process_api_requests_from_file(
    ...
    # read save_filepath, if it exists, to potentially skip certain requests
    os.makedirs(os.path.dirname(save_filepath), exist_ok=True)
    if os.path.exists(save_filepath):
        df = pd.read_json(save_filepath, lines=True)
        ...
        # set status_tracker.completed_requests with this information
        status_tracker.completed_requests = success_df.to_dict(orient='index')
                ...
                if not queue_of_requests_to_retry.empty():
                    ...
                    try:
                        ...
                        file_id, unique_id, messages = chunk_dict[
                            'conversation_id'], chunk_dict['chunk_id'], chunk_dict[
                                'messages']
                        request_json = {
                            "model": model,
                            "messages": messages,
                        ...
                        # potentially skip this request if it was already completed
                        if (file_id, unique_id) in status_tracker.completed_requests and \
                            status_tracker.completed_requests[(file_id, unique_id)]['success']:
                            logging.info(
                                f"Skipping request {next_request.task_id}. Already completed."
                            )
                            next_request = None
                            continue
...
class StatusTracker:
    ...
    completed_requests: Dict = field(default_factory=dict) # maps (file_id, unique_id) to {'success': True/False}, indicating whether the (file_id, unique_id) request has been completed
...
def append_to_jsonl(data, filename: str) -> None:
    ...
    # ensure filename's directory exists
